...
(async () => {
    const rest = new REST({ version: "9" }).setToken(config.getData("/d_token"));
    ...
    await rest.put(
        Routes.applicationCommands(config.getData("/d_app_id")),
        { body: cmds.everyone }
    );
    await rest.put(
        Routes.applicationGuildCommands(config.getData("/d_app_id"), config.getData("/d_admin_server_id")),
        { body: cmds.admin }
    );
